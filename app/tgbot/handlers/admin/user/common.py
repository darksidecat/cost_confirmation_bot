...
async def get_users(dialog_manager: DialogManager, uow: IUserUoW, **kwargs):
    users = await GetUsers(uow)()
    return {USERS: users}
...
async def get_user(dialog_manager: DialogManager, uow: IUserUoW, **kwargs):
...
async def get_user_data(dialog_manager: DialogManager, uow: IAccessLevelUoW, **kwargs):
    dialog_data = dialog_manager.current_context().dialog_data

    levels = []
    levels_ids = dialog_data.get(ACCESS_LEVELS)
    if levels_ids:
        all_levels = await GetAccessLevels(uow)()
